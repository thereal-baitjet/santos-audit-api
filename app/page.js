...
const homepageJsonLd = {
  ...
  offers: [
    ...
    { "@type": "Offer", name: "Structured Extraction", price: process.env.STRUCTURED_EXTRACT_PRICE_USDC ?? "0.08", priceCurrency: "USDC", url: "https://api.santosautomation.com/v1/extract/structured" },
    { "@type": "Offer", name: "Feed Parser", price: process.env.FEED_PRICE_USDC ?? "0.003", priceCurrency: "USDC", url: "https://api.santosautomation.com/v1/feed" },
    { "@type": "Offer", name: "Link Map", price: process.env.LINKS_PRICE_USDC ?? "0.003", priceCurrency: "USDC", url: "https://api.santosautomation.com/v1/links" },
    { "@type": "Offer", name: "Summarizer", price: process.env.SUMMARIZE_PRICE_USDC ?? "0.033", priceCurrency: "USDC", url: "https://api.santosautomation.com/v1/summarize" },
    ...
    { "@type": "Offer", name: "Screenshot & PDF Render", price: process.env.SCREENSHOT_PRICE_USDC ?? "0.01", priceCurrency: "USDC", url: "https://api.santosautomation.com/v1/screenshot" },
    { "@type": "Offer", name: "Batch Quick Intelligence Audit", price: process.env.BATCH_AUDIT_PRICE_USDC ?? "0.50", priceCurrency: "USDC", url: "https://api.santosautomation.com/api/audit/batch" },
    { "@type": "Offer", name: "Quick Agent Readiness Report by card (human checkout)", price: "9", priceCurrency: "USD", url: "https://www.santosautomation.com/agent-readiness/buy" },
  ],
};
const founderJsonLd = {
  "@context": "https://schema.org",
  ...
  address: { "@type": "PostalAddress", addressLocality: "Union City", addressRegion: "NJ", addressCountry: "US" },
    ...
    <PageShell>
      ...
      <header className="hero">
        <div className="hero-copy">
          <p className="kicker">x402 · USDC on Base · No account</p>
          <h1>Is your website ready for the <em>agentic web</em>?</h1>
          <p className="lede">Measure whether any website can be discovered, understood, trusted and used by AI agents. Santos audits MCP, llms.txt, OpenAPI, structured metadata, crawler access, SEO, accessibility, performance and security — then returns structured JSON with evidence and prioritized fixes.</p>
          ...
          <div className="cta-row">
            ...
            <a className="btn" href="/agent-readiness/buy">Get a report — $9</a>
      ...
      <div className="stat-strip">
        <div className="wrap">
          <div className="stat"><b>11</b><span>paid capabilities</span></div>
          <div className="stat"><b>$0.002</b><span>entry price per call</span></div>
          <div className="stat"><b>50</b><span>URLs per batch</span></div>
          <div className="stat"><b>0</b><span>accounts or API keys</span></div>
        </div>
      </div>

      <div className="home-content">
        <section className="content-section" id="index"><p className="section-label">The Santos Index · July 2026</p><h2>The web isn't agent-ready — yet.</h2><p className="sub wide">We audited 200+ of the world's best-known websites with the same engine that powers this API. Average AI Website Intelligence score: <strong>59/100</strong>. Median: <strong>57</strong>. google.com scores <strong>37</strong>, oracle.com <strong>39</strong>, binance.com <strong>24</strong> — while planetscale.com leads at <strong>91</strong>. Every report is public, HMAC-signed, and independently verifiable.</p><div className="cta-row"><a className="btn primary" href="/reports">Browse the leaderboard</a><a className="btn" href="#audit">Score your own site free</a></div></section>
        ...
        <section className="content-section" id="methodology"><p className="section-label">07 · Methodology</p><h2>Scored by applicability, grounded in public evidence</h2><div className="prose-grid"><div><h3>Classify first</h3><p>General websites, documentation, APIs, MCP providers, and commerce services do not need the same interfaces. Non-applicable categories leave the denominator.</p></div><div><h3>Expose coverage</h3><p>Passed, failed, unknown, skipped, and not-applicable states stay visible alongside confidence and the exact discovered interfaces.</p></div><div><h3>Keep scoring deterministic</h3><p>Published rules and weights drive numeric results. AI summaries, when requested in Deep audits, never change scores or pass/fail status.</p></div><div><h3>State the limits</h3><p>Public, passive, single-page evidence is not certification, a vulnerability scan, or a promise of visibility in AI-generated answers.</p></div></div><div className="cta-row"><a className="btn" href="/methodology/agent-readiness">Read the full scoring methodology</a><a className="btn" href="/learn/agent-ready-website-checklist">Use the agent-ready checklist</a></div><div className="guide-links" aria-label="Website Intelligence field guides"><a href="/learn/what-is-ai-website-intelligence">What is AI Website Intelligence?</a><a href="/learn/what-is-agent-readiness">What makes a website agent-ready?</a><a href="/learn/mcp-vs-openapi">MCP vs OpenAPI</a><a href="/learn/llms-txt-vs-robots-txt">llms.txt vs robots.txt</a><a href="/learn/how-ai-agents-discover-capabilities">How agents discover capabilities</a><a href="/learn/structured-data-for-ai-agents">Structured data for AI agents</a><a href="/learn/from-discoverable-to-callable">From discoverable to callable</a><a href="/learn/agent-ready-website-checklist">Agent-ready website checklist</a><a href="/learn/x402-explained">x402 explained</a><a href="/learn/four-dimensions-of-agent-readiness">The four dimensions, scored</a><a href="/learn/capability-manifests">Capability manifests</a><a href="/learn/what-200-audits-reveal">What 200+ audits reveal</a><a href="/learn/santos-vs-lighthouse-vs-seo-tools">Santos vs Lighthouse vs SEO tools</a><a href="/learn/planetscale-vs-google-score-teardown">planetscale 91 vs google 39</a></div></section>

        <section className="content-section split-section" id="about"><div><p className="section-label">08 · About the builder</p><h2>Built and operated by one accountable engineer</h2><p className="sub wide">Santos Automation is founded and run by <strong>Juan Santos</strong>. He spent years as a hands-on field technician and in sales before teaching himself software engineering — since 2020 he has shipped more than 100 public projects, from e-commerce with live Stripe checkout to AI audit systems and automation tooling. He now runs Santos Automation full-time from Union City, New Jersey.</p><p className="sub wide">This API is production infrastructure, not a pitch deck: it meters real payments in USDC on Base, publishes its scoring methodology, and the same engine has audited 200+ of the world's best-known websites — every report HMAC-signed and independently verifiable.</p><p className="sub wide">No ticket queue, no account managers. When you contact Santos, you reach the person who wrote the code.</p><div className="inline-links" aria-label="Founder links"><a href="https://github.com/thereal-baitjet" rel="me">GitHub</a><a href="https://www.linkedin.com/in/santosjuanc/" rel="me">LinkedIn</a><a href="mailto:info@santosautomation.com">info@santosautomation.com</a></div></div><img className="about-portrait" src="/assets/santos-portrait.png" alt="Portrait of Juan Santos, founder and engineer of Santos Automation" width="1086" height="1448" loading="lazy" /></section>

        <section className="content-section audit" id="audit"><p className="section-label">09 · Live entry point</p><h2>Free browser demo — one scan per day</h2><p className="sub wide">This is the free daily demo (one scan per day per verified email — confirm your address once with a 6-digit code, then it just works). It inspects the report shape only. For unlimited synchronous audits use the production Quick API at 0.015 USDC per success, and for bounded agent-interface evidence run the <a href="/agent-readiness/run">paid Agent Readiness audit</a> at {readinessPrice} USDC per success. The same daily quota also covers one free <a href="/website-intelligence-api">page-to-Markdown extraction</a> at GET /v1/extract/demo.</p><div className="audit-box"><AuditWidget /></div></section>

        <section className="content-section" id="pricing" data-analytics-event="pricing_viewed"><p className="section-label">10 · Transparent pricing</p><h2>Pricing built for agents</h2><p className="sub wide">No subscription and no traditional API key. Eleven x402 capabilities on Base (no account), from hardened raw fetches at <strong>$0.002 USDC</strong> to browser-rendered reports at <strong>$0.225 USDC</strong>, with flat-rate batch audits up to 50 URLs. For humans, there are also optional one-time <a href="/agent-readiness/buy">card reports, formatted, emailed, and verifiable</a> — human reports from $9 (Quick) to $29 (Deep, browser-rendered), or <a href="/monitoring">monitor any URL weekly for $9/mo</a> — weekly re-audit, an alert when your score moves 5+ points, and a monthly digest. x402 prices are the agent price (raw API); card prices are the human report (formatted, emailed, verifiable). x402 terms are returned before payment, and the audited target is never paid by Santos during discovery.</p><div className="plan-grid"><article className="plan plan--fetch"><div className="plan-head"><h3>Safe Fetch</h3><div className="plan-price">$0.002 <span>USDC / success</span></div><div className="plan-mode">GET /v1/fetch</div></div><p className="plan-pitch">One URL in, raw body out through an SSRF-guarded fetcher — redirects, headers, and timing included.</p></article><article className="plan plan--extract"><div className="plan-head"><h3>Content Extraction</h3><div className="plan-price">$0.005 <span>USDC / success</span></div><div className="plan-mode">POST /v1/extract</div></div><p className="plan-pitch">One public page in, clean Markdown out — title, links, and metadata for RAG and research agents.</p></article><article className="plan"><div className="plan-head"><h3>Feed Parser</h3><div className="plan-price">$0.003 <span>USDC / success</span></div><div className="plan-mode">GET /v1/feed</div></div><p className="plan-pitch">Any RSS 2.0, Atom, or JSON Feed URL in, normalized JSON items out — titles, links, dates, summaries, capped at 50.</p></article><article className="plan"><div className="plan-head"><h3>Link Map</h3><div className="plan-price">$0.003 <span>USDC / success</span></div><div className="plan-mode">GET /v1/links</div></div><p className="plan-pitch">One page in, every link out — resolved, deduplicated, and tagged internal, external, docs, pricing, API, careers, social, or feed.</p></article><article className="plan"><div className="plan-head"><h3>Quick Intelligence</h3><div className="plan-price">$0.015 <span>USDC / success</span></div><div className="plan-mode">GET /api/audit</div></div><p className="plan-pitch">Fast single-page performance, SEO, accessibility markup, security, and structured fixes.</p></article><article className="plan plan--agent"><div className="plan-head"><h3>Agent Readiness</h3><div className="plan-price">${readinessPrice} <span>USDC / success</span></div><div className="plan-mode">GET /api/agent-readiness · or from $9 by card</div></div><p className="plan-pitch">Bounded passive discovery of agent documentation, identity, OpenAPI, MCP, trust, and commerce. The main path for agents is <strong>{readinessPrice} USDC via x402</strong>; humans can optionally <a href="/agent-readiness/buy">buy a report by card — $9 Quick or $29 Deep</a>.</p></article><article className="plan plan--shot"><div className="plan-head"><h3>Screenshot &amp; PDF</h3><div className="plan-price">$0.01 <span>USDC / render</span></div><div className="plan-mode">GET /v1/screenshot</div></div><p className="plan-pitch">Real-Chromium PNG, JPEG, or PDF of any public page — JavaScript rendered, desktop or mobile, viewport or full page. Verify deploys, monitor changes, archive evidence, or feed vision models.</p></article><article className="plan plan--structured"><div className="plan-head"><h3>Structured Extraction</h3><div className="plan-price">$0.08 <span>USDC / success</span></div><div className="plan-mode">POST /v1/extract/structured</div></div><p className="plan-pitch">One page and your own JSON Schema in, LLM-extracted JSON validated against that schema out — never charged unless the output actually conforms.</p></article><article className="plan"><div className="plan-head"><h3>Summarizer</h3><div className="plan-price">$0.033 <span>USDC / success</span></div><div className="plan-mode">POST /v1/summarize</div></div><p className="plan-pitch">One HTML page in, a Claude brief out — title, ~200-word summary, key facts, and named entities. Optional focus steers the summary.</p></article><article className="plan plan--batch"><div className="plan-head"><h3>Batch Audit</h3><div className="plan-price">$0.50 <span>USDC / up to 50 URLs</span></div><div className="plan-mode">POST /api/audit/batch</div></div><p className="plan-pitch">Fifty full quick audits in one flat payment — portfolios, prospect lists, whole sites. Per-URL failure isolation; settles only when at least one audit succeeds.</p></article><article className="plan plan--deep"><div className="plan-head"><h3>Deep Intelligence</h3><div className="plan-price">$0.225 <span>USDC / reservation</span></div><div className="plan-mode">POST /v1/audits</div></div><p className="plan-pitch">Browser-rendered Lighthouse, axe-core, screenshots, network evidence, and passive security.</p></article><article className="plan plan--human"><div className="plan-head"><h3>Quick Report</h3><div className="plan-price">$9 <span>one-time · by card</span></div><div className="plan-mode">Formatted report · emailed same-day</div></div><p className="plan-pitch">Quick fetch-based evidence of any URL's agent readiness, formatted and emailed same-day. The human version of the $0.075 agent call.</p><a className="btn primary" href="/agent-readiness/buy">Buy a report →</a></article><article className="plan plan--human"><div className="plan-head"><h3>Deep Report</h3><div className="plan-price">$29 <span>one-time · by card</span></div><div className="plan-mode">Browser-rendered · emailed when ready</div></div><p className="plan-pitch">Browser-rendered Lighthouse, axe-core, screenshots, and network evidence, formatted and emailed when ready.</p><a className="btn primary" href="/agent-readiness/buy">Buy a report →</a></article><article className="plan plan--human"><div className="plan-head"><h3>Monitoring</h3><div className="plan-price">$9 <span>/mo · by card</span></div><div className="plan-mode">Weekly re-audit · cancel anytime</div></div><p className="plan-pitch">Weekly re-audit, an email alert when your score moves 5+ points, monthly digest. Cancel anytime.</p><a className="btn primary" href="/monitoring">Start monitoring →</a></article></div><div className="cta-row"><a className="btn primary" href="/agent-readiness/run">Run Paid Agent Readiness Audit</a><a className="btn" href="#audit" data-analytics-event="free_audit_started">Run Free Quick Audit</a><a className="btn" href="/website-intelligence-api">Integrate the API</a></div></section>

        <section className="content-section" id="faq"><p className="section-label">11 · FAQ</p><h2>Questions about AI Website Intelligence</h2><div className="faq-list">{FAQS.map((item) => <details key={item.question}><summary>{item.question}</summary><p>{item.answer}</p></details>)}</div></section>
      </div>
    </PageShell>
  );
}
