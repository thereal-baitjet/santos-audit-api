...
export function track(event, props) {
  try {
    ...
    // ePrivacy: no analytics until the visitor has accepted via the consent
    // banner (see app/components/CookieConsent.js).
    if (!/(?:^|; )santos_consent=all(?:;|$)/.test(document.cookie)) return;
...
// events without per-component code. Also forwards `santos:analytics`
// CustomEvents dispatched by non-React scripts (e.g. public/audit-widget.js).
// Call once from a client boundary.
export function bindAnalyticsAttributes() {
  ...
  window.addEventListener("santos:analytics", (e) => {
    track(e.detail?.name);
  }, { passive: true });
}
