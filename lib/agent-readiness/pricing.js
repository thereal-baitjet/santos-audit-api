...
function findContradictions(claims, challengeClaim) {
  ...
  for (const [scope, scopedClaims] of groups) {
    // "Enforced" means the terms the resource actually challenges with. Without
    // a challenge for this scope there is nothing to contradict: one URL may
    // legitimately advertise several prices — a checkout page selling a $9 and
    // a $29 report, or a landing page listing a whole catalog. Treating the
    // first claim as enforced reported every additional product at that URL as
    // a pricing contradiction.
    const enforced = scopedClaims.find((claim) => claim.source === "x402_challenge");
