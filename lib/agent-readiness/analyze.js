...
export async function auditAgentReadiness(rawUrl, options = {}) {
  ...
  assessRoot({ $, text, finalUrl, advertised, jsonLd, signals, add });
  ...
  if (budget.remaining > 0 && options.mode !== "embedded") {
    ...
    if (llms.ok && llms.response.status === 200) {
      ...
      signals = {
        ...
        commerce: signals.commerce || /\b(x402|usdc|machine[- ]payable|micropayment)\b/.test(lower),
      };
  ...
  if (applicability.agent_commerce === "tested") {
    ...
    const paidCandidate = findPaidCandidate(advertised, canonicalOrigin);
    add("agent.commerce.discovery", paidCandidate ? "pass" : "fail", paidCandidate ? "A canonical paid resource is advertised" : "Machine payment is claimed without a canonical paid resource", paidCandidate ? { resource_url: redactUrl(paidCandidate.url) } : {}, paidCandidate ? "info" : "high");
    if (paidCandidate && budget.remaining > 0 && options.mode !== "embedded") {
      const challenge = await inspectPaymentChallenge(paidCandidate.url, budget);
...
function collectAdvertisedUrls($, headers, baseUrl) {
  ...
  $("a[href], link[href]").each((_, element) => {
    ...
    try {
      ...
      if (["http:", "https:"].includes(url.protocol)) found.push({ url: url.href, source: element.tagName === "link" ? "html-link" : "html-anchor", rel: $(element).attr("rel") ?? "" });
...
function classifySignals({ $, text, advertised, jsonLd }) {
  ...
  const commerce = commerceEvidence.length > 0;
...
function assessRoot({ $, text, finalUrl, advertised, jsonLd, signals, add }) {
  ...
  if (signals.commerce) add("agent.jsonld.offer", jsonLd.types.includes("Offer") ? "pass" : "fail", jsonLd.types.includes("Offer") ? "Machine-readable offer metadata is present" : "Paid access is advertised without an Offer", { types: jsonLd.types }, jsonLd.types.includes("Offer") ? "info" : "low");
...
async function inspectPaymentChallenge(url, budget) {
  ...
  return { valid, evidence: { http_status: fetched.response?.status ?? null, x402_version: terms?.x402Version ?? null, scheme: offer?.scheme ?? null, network: offer?.network ?? null, amount: offer?.amount ?? null, asset: offer?.asset ?? null, resource_url: redactUrl(terms?.resource?.url ?? url), payment_signature_sent: false } };
}
...
function findMcpCandidate(advertised, origin) {
  ...
  return item ? { ...item, confidence: item.source === "llms.txt" || /mcp/i.test(item.rel) ? "high" : "medium" } : null;
}

function findPaidCandidate(advertised, origin) {
  const item = advertised.find((candidate) => isRelatedServiceUrl(candidate.url, origin) && /\/api\/audit\?|\/v1\/audits(?:\?|$)/i.test(candidate.url));
  return item ?? null;
}
