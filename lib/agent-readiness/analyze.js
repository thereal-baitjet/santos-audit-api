...
const DEFAULT_UA = "SantosAgentReadinessBot/1.0 (+https://api.santosautomation.com/llms.txt; support: info@santosautomation.com)";
...
export async function auditAgentReadiness(rawUrl, options = {}) {
  ...
  if (applicability.mcp_readiness === "tested") {
    if (mcpCandidate) {
      ...
      if (config.registryLookup && budget.remaining > 0 && options.mode !== "embedded") {
        ...
        const reachable = registry.evidence?.status !== "unavailable" && registry.evidence?.status !== "invalid_response";
        add(
          "agent.mcp.registry",
          registry.matched ? "pass" : "unknown",
          registry.matched
            ? "A matching official MCP Registry entry was found"
            : reachable
              ? "No entry for this domain was found in the official MCP Registry"
              : "The official MCP Registry could not be reached, so listing could not be established",
          registry.evidence,
          "info"
        );
      } else {
  ...
  return assertAgentReadinessResult({
    ...
    limitations: [
      "Passive public-surface assessment only; the auditor did not authenticate to or pay the target, create a target account, submit forms, or invoke business tools.",
...
/**
 * The MCP Registry namespaces servers by reverse DNS — santosautomation.com
 * publishes as `com.santosautomation/site-audit` — and its search matches that
 * form, not the dotted hostname. Searching "santosautomation.com" returns
 * nothing; "com.santosautomation" returns the entry.
 */
export function registryNamespaceFor(hostname) {
  const host = String(hostname ?? "").trim().toLowerCase().replace(/^www\./, "").replace(/\.$/, "");
  if (!host || !host.includes(".")) return null;
  return host.split(".").filter(Boolean).reverse().join(".");
}
...
async function lookupMcpRegistry(hostname, budget, config) {
  ...
  const namespace = registryNamespaceFor(hostname);
  if (!namespace) return { matched: false, evidence: { status: "unresolvable_hostname" } };

  // Search rather than page. The previous implementation fetched an unsorted
  // first 100 servers and checked whether the domain happened to appear in it,
  // so a correctly published server was reported missing unless it landed in
  // that arbitrary page — a false negative for every registry beyond 100 entries.
  const url = `${config.registryBaseUrl.replace(/\/$/, "")}/v0/servers?search=${encodeURIComponent(namespace)}&limit=100`;
  const fetched = await budget.tryFetch(url, {
    purpose: "mcp-registry",
    allowCrossOrigin: true,
    allowedContentTypes: JSON_TYPES,
    maxResponseBytes: 750_000,
    // A cold TLS handshake to the registry regularly exceeds the 8s site
    // default, which reported a published server as missing.
    timeoutMs: Math.max(config.timeoutMs, 15_000),
  });
  if (!fetched.ok || fetched.response.status !== 200) {
    return { matched: false, evidence: { registry_url: redactUrl(url), searched_namespace: namespace, http_status: fetched.response?.status ?? null, status: "unavailable" } };
  }
  let doc;
  try { doc = JSON.parse(fetched.body); } catch { return { matched: false, evidence: { registry_url: redactUrl(url), searched_namespace: namespace, status: "invalid_response" } }; }

  // A search hit is not proof of ownership: confirm each candidate really
  // belongs to this domain, either by namespace or by naming the host itself.
  ...
  const matches = servers
    ...
    .filter((server) => {
      ...
      if (name === namespace || name.startsWith(`${namespace}/`) || name.startsWith(`${namespace}.`)) return true;
      return JSON.stringify(server).toLowerCase().includes(host);
    })
    .slice(0, 5);
  ...
  const value = {
    ...
    evidence: {
      registry_url: redactUrl(url),
      registry_status: "preview",
      searched_namespace: namespace,
      results_returned: servers.length,
      matches: [...new Set(matches.map((server) => server.name).filter(Boolean))],
      cached: false,
    },
  };
...
// Documentation and specification artifacts describe paid resources; they are
// never the paid resource itself. Probing one for a payment challenge always
// returns 200 and produces a false "did not return a valid passive challenge"
// failure against a service whose real endpoints challenge correctly.
const SPEC_ARTIFACT_PATTERN =
  /(?:^|\/)(?:openapi(?:\.json|\.ya?ml)?|swagger(?:\.json|\.ya?ml)?|llms(?:-full)?\.txt|capabilities\.json|agent-capabilities\.json|sitemap\.xml|robots\.txt|ai-plugin\.json)$|\/\.well-known\/|\.(?:md|txt|ya?ml)$/i;

function isSpecArtifactUrl(value) {
  try {
    return SPEC_ARTIFACT_PATTERN.test(new URL(String(value)).pathname);
  } catch {
    return false;
  }
}

// Documented URLs often carry a display placeholder ("?url=…") that must not be
// sent to the origin as a literal query.
function stripUrlPlaceholder(value) {
  try {
    const parsed = new URL(String(value));
    if (/(?:…|%E2%80%A6|\{[^}]*\}|\.\.\.)/.test(parsed.search)) parsed.search = "";
    return parsed.href;
  } catch {
    return value;
  }
...
function findPaidCandidate(advertised, origin, pricingClaims = []) {
  const usable = (url) => url && isRelatedServiceUrl(url, origin) && !isSpecArtifactUrl(url);

  // Strongest signal: a URL shaped like a known paid route.
  const byRoute = advertised.find((candidate) => usable(candidate.url)
    && /\/api\/audit\?|\/v1\/audits(?:\?|$)/i.test(candidate.url));
  if (byRoute) return { ...byRoute, url: stripUrlPlaceholder(byRoute.url), method: "GET" };

  // Next: something the service actually attaches an x402 price to. This
  // outranks the label heuristic below — a real priced resource is far better
  // evidence than prose, and a marketing link described as a "paid audit
  // handoff" was being probed as though it were the paid endpoint itself.
  const machinePricedGet = pricingClaims.find((claim) => usable(claim.resource_url)
    && claim.amount != null
    && (!claim.method || claim.method === "GET")
    && /^x402(?:-|$)/i.test(claim.protocol ?? ""));
  if (machinePricedGet) {
    return {
      url: stripUrlPlaceholder(machinePricedGet.resource_url),
      source: machinePricedGet.source,
      method: machinePricedGet.method ?? "GET",
    };
  }

  // Weakest: a link whose label merely talks about payment.
  const byLabel = advertised.find((candidate) => usable(candidate.url)
    && /\b(x402|paid|pay|purchase|buy)\b/i.test(`${candidate.rel ?? ""} ${candidate.label ?? ""}`));
  return byLabel ? { ...byLabel, url: stripUrlPlaceholder(byLabel.url), method: "GET" } : null;
}
