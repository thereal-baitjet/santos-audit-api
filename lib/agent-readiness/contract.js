...
export const AGENT_READINESS_RESULT_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    // Inlined (no $ref/$defs): this schema is embedded verbatim inside the
    // 402's extensions.bazaar output schema, where a root-anchored "#/$defs/…"
    // ref resolves against the embedding document and breaks registry probers.
    findings: {
      type: "array",
      items: {
        type: "object",
