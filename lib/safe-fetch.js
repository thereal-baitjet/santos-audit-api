...
// and caps redirects, time, and response size.
...
async function assertPublicHost(hostname) {
  ...
  if (isIP(bare)) {
    if (isPrivateIp(bare)) throw new AuditError("PRIVATE_ADDRESS_BLOCKED", "Private or reserved IP addresses are not allowed");
    return;
  }
  let addrs;
  try {
...
async function readCapped(res) {
  ...
  if (declared && declared > MAX_RESPONSE_BYTES) {
    throw new AuditError("AUDIT_FAILED", `Response exceeds ${MAX_RESPONSE_BYTES} byte limit`);
  }
...
// Cheap synchronous validation (no DNS): parseability, scheme, and literal
// local/private hosts. Lets routes reject obvious garbage before rate limiting.
...
export async function safeFetch(rawUrl, headers) {
  let url = validateTarget(rawUrl);
  const deadline = AbortSignal.timeout(AUDIT_TIMEOUT_MS);
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    let res;
    try {
      res = await fetch(url, { headers, redirect: "manual", signal: deadline });
    } catch (e) {
    ...
    if (res.status >= 300 && res.status < 400 && res.headers.get("location")) {
      ...
      url = new URL(res.headers.get("location"), url); // relative-safe; next loop revalidates
      continue;
    }
  ...
  throw new AuditError("AUDIT_FAILED", `Exceeded ${MAX_REDIRECTS} redirects`);
}
