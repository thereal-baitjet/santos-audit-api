...
function enterDegraded(error) {
  ...
  // not turn into an alert storm that gets muted.
  const now = Date.now();
  if (now - lastAlertAt > DEGRADED_ALERT_COOLDOWN_MS) {
    lastAlertAt = now;
    import("../notify.js")
      .then(({ notifyOpsAlert }) =>
        notifyOpsAlert({
          title: "Free-tier limiter degraded",
          detail:
            `The rate-limit store is unreachable, so free-tier quota cannot be enforced durably. ` +
            `Falling back to ${EMERGENCY_MAX} grant(s) per key per instance and denying past that. ` +
            `Heavy free work (LLM-backed extraction) is refused entirely while degraded. ` +
