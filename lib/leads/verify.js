...
// Send a fresh 6-digit code. Always resolves { ok:true } so the route never
// leaks which addresses exist or whether a limit was hit; dev_code is
// included only in non-production without a Resend key (local testing).
export async function requestCode({ email, targetUrl, source, ip }) {
  ...
  const result = { ok: true, sent: sent.ok };
